...
def potential_field_control_tp5(lidar, current_pose, goal_pose, main_goal = True):
    """
    Control using potential field for goal reaching and obstacle avoidance
    lidar : placebot object with lidar data
    current_pose : [x, y, theta] nparray, current pose in odom or world frame
    ...
    # control function for going to the main goal
    if main_goal:
        # system parameters
        d_change = 40
        stop_dist = 5
        ...
        # control function for going back to start
        # here we use the potential_field_control but ignore the obstacle because no obstacle will be on the path
